# Version 7.2.3

<https://documentation.neutrinos.com/articles/#!app-builder-s-user-guide/new-in-version-7-2-3>

Here is a list of new enhancements introduced in Neutrinos Studio 7.2.3

**Enhancements**

- From this release of Neutrinos Studio, a search feature is added to search for particular **Pages**, **Services**, and **Models **in its respective lists. Search can be done by clicking the search icon or use the short cut **CTRL+F** when the list is open.
   ![search function](/resources/Storage/app-builder-s-user-guide/search.png)
- Added default client env variable NGFORAGE_MOBILE_DRIVER which can take values `WEB_SQL`, `INDEXED_DB`, `LOCAL_STORAGE.`
- Neutrinos-module upgraded to version 0.0.59 to add the NGFORAGE_MOBILE_DRIVER environment property.
- Added a **Download File** option for the **HTTP Out** node of the Server Services Designer.
- The initial loading time of the services in the Services Designer is improved by 95%.
- Tooltip added to show the entire name of the services in the Services designer.
- Added click event property in the **Menu **component.
- The **required **field of the Form fields is set to **false **by default.

Refer to the [ChangeLog](/smart/project-change-log/release-7-2-3) to learn about the bug fixes.
