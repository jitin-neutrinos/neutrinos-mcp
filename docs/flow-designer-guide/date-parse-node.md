...
<https://documentation.neutrinos.com/articles/#!flow-designer-guide/date-parse-node>

This is the default topic template.

The **Parse** node is used to parse date and time as a JavaScript object, string, array, and date object. It returns the moment object as an output.

### Node Properties

**Name:** A unique name for the node.**Function Name: **This is a read-only field. The function name is generated based on the name that you enter in the Name field. It is used to identify the node while debugging.**Action:** The action to be performed on the parameters. See [Parse actions](/articles/flow-designer-guide/date-parse-node/a/h3__929851382) to learn about the actions that can be performed using this node. Depending on the action you choose, the fields to configure will differ.**Result Mapping: **Returns a moment object or a UTC time moment object based on the action you choose. You can map the result to a [flow or page variable](/articles/flow-designer-guide/properties-page-designer).
...
**Date**: The date (in a string) to be parsed. Example - 21/02/2021.
