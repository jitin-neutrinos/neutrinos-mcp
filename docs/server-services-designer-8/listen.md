...
<https://documentation.neutrinos.com/articles/#!server-services-designer-8/listen>

The **Listen node** allows you to listen to the event emitted by the [Emit node](/articles/server-services-designer-8/emit). It is a **s****tart **node, but does not initialize the .bh object. Instead, it accepts the .bh object from the** Emit **node.
