...
<https://documentation.neutrinos.com/articles/#!how-to-articles/working-with-databases>

Neutrinos integrates with your existing NoSQL databases. This allows you to develop applications that access data on your external database without having to worry about data migration.
...
4. Double-click the node to open its **Properties** window.
  ...
  3. Enter the path as** mongoinsert**. Click the **Done **icon to save the properties.
      ![](/resources/Storage/how-to-articles/mong_httpin.png)
5. Drag and drop a **MongoDB** node next to the **HTTP IN** node and connect the nodes to create a flow. Double-click the **MongoDB **node and configure the node.
  ...
  2. Click ![add database config](/resources/Storage/how-to-articles/add_db_config.png) to add a new MongoDB connection. In the **DB Config Properties** page, enter the following details:
    ...
    3. Click the **Done** icon.
        ![MongoDB configuration](/resources/Storage/how-to-articles/mong_conf.png)
  ...
  6. In the **Result mapping **field, select bh.input and enter the variable name as **res**. The response of the operation will be saved in this variable. Click the **Done **icon to save the properties.
      ![Mongo DB node configuration](/resources/Storage/how-to-articles/mongo_props.png)
6. Drag and drop an **HTTP Response** node next to the **Mongo DB** node and connect the nodes to create the service flow. The flow should look like:
    ![server flow to insert a record in Mongo DB](/resources/Storage/how-to-articles/mongo_flow.png)
...
8. Now when you [call the server flow from the application](/smart/project-how-to-articles/call-a-server-flow-from-page), the document entered in the **MongoDB **node gets inserted into the database.
