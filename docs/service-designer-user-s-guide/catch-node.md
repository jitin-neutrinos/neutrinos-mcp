...
<https://documentation.neutrinos.com/articles/#!service-designer-user-s-guide/catch-node>
...
| ![Information](/resources/Storage/service-designer-user-s-guide/info.png) | In a flow, if an exception occurs on the node for which the catch node is not enabled, its exception will be passed to its previous node in the flow. |
| --- | --- |

If a node throws an error, the catch node catches the **error** object which is available inside the bh.error property. See [Properties in client Services](/articles/service-designer-user-s-guide/service-designer-variables) to learn more about the error property.
