...
<https://documentation.neutrinos.com/articles/#!service-designer-user-s-guide/cursor-node>

The **Cursor **node is used to iterate an index or an object within the [IndexDB database](/articles/service-designer-user-s-guide/indexed-db-node) using a key range. An object store is sorted internally by key. Given a query, a cursor traverses the object-store and returns one key/value at a time.

Cursors are used as an alternate to the [Get](/articles/service-designer-user-s-guide/get-node) operation (which returns an array of keys/values). If the object storage is bigger than the available memory, then the **Get** operation will fail to get all records as an array. In such situations, cursors are used to traverse the object storage, given a query, and return one key/value at a time, thus saving memory.
...
3. **Store Name:** The store on which you want to perform transactions. Enter the [flow object](/articles/service-designer-user-s-guide/service-designer-variables)containing the object store name.
