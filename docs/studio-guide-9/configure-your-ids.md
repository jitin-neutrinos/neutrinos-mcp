...
<https://documentation.neutrinos.com/articles/#!studio-guide-9/configure-your-ids>

To authenticate and authorize the users of your application, you should register your app with an Identity Server (IDS). You can opt to use the Neutrinos Auth Strategy or connect to external OAuth providers such as Active Directory, Google, or Azure AD to secure your app.

| ![Information](/resources/Storage/studio-guide-9/app-builder-s-user-guide/info.png) | If you have an on-premises or Private cloud IDS setup, then you should have a domain name with HTTPS to establish a secure connection. Public IPs cannot be configured with Azure AD or Google accounts. |
| --- | --- |

### General IDS Settings

To enable IDS on your app, perform the following:

1. Click **Settings** > **IDS o**n the [Studio Application](/smart/project-concepts/studio-application-page) page.
...
3. Select **Skip Consent **if you already have an active session with the IDS and want the app to skip showing the IDS consent page.
    ![IDS consent page](/resources/Storage/studio-guide-9/ids%20consent%20page.png)
...
7. **Login Redirect URI**: A login redirect URI (Uniform Resource Identifier) is where the client is redirected after the authorization is successful. This is required to obtain the token set and user information.
    By default, Neutrinos provides a URI where the server services are running locally in your machine. These server services are the client of the IDS. You can also add additional Redirect URIs depending on where your server services are running. To add a login redirect URI, click the **+** icon and provide the URI.
...
Configure the Identity Server for your app using the following strategies:

- [Use Neutrinos Authentication](/articles/studio-guide-9/configure-ids-with-neutrinos)
- [Use Active Directory Authentication provider](/articles/studio-guide-9/configure-ids-with-active-directory)
- [Use Google Authentication Provider](/articles/studio-guide-9/configure-ids-with-google)
...
The organisation admins should perform [user management](/articles/app-builder-s-user-guide/perform-user-management)on [Neutrinos Console](/articles/concepts-publication/neutrinos-console) to define which users get to access the application.
