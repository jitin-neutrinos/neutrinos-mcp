...
<https://documentation.neutrinos.com/articles/#!server-services-designer-9/http-in>
...
2. **Use Datamodel Toggle Button: **If the toggle is enabled, HTTP In node can be used for creating service flows for Data Model operations. HTTP In node can further be associated with DM nodes to create custom CRUD request.
3. **Method:** It specifies what the client would like the server to do with the specified resource. Select a method from the drop-down list. HTTP methods include:
...
4. **Path:** The URI path on which you want to perform the HTTP operations. For example, if the path is /file-upload/:jdf then the complete path will be localhost:8081/api/file-upload/:jdf
5. **Middleware Sequence:** The middleware sequence that you want to use for this **HTTP In** node. It has a dropdown where you can select the middleware sequence that is configured in the middleware's section of the services. The middleware sequences are denoted with the sequence id's or by their names in the list. See Configuring middleware to know more.
6. **Select Data Model:** Configure the fields in the section to use HTTP in node to create data model services. These fields only appears when the Use Datamodel toggle is on.![](/resources/Storage/server-services-designer-9/server-services-designer-preface-2022-03-31.png)

- **Accept List of the Selected Entities:** When the toggle is enabled, the HTTP In node can insert multiple records/attributes for a selected entity.
- **Data Model: **Displays a list of data models that you can use for HTTP In node.
- **Entity:** Based on the selected Data Model, this attribute displays a list of entities pertaining to the data model.
- Selected Entity's Attributes: Displays a list of attributes in the selected entity. This is a read-only property.

1. **File Upload:** Configure the fields in this section to accept the file uploads. To do so, toggle "**A****ccept File Upload?**" to true. These fields appear only when you set the file upload to true.

- **Destination**: Defines where the file should be saved after uploading. Choose **File Path** or **Memory**. The drop-down options change based on your selection:![File upload in http](/resources/Storage/server-services-designer-9/file_upload.png)
...
- **Summary: **The summary of the API endpoint.![Documentation of http in](/resources/Storage/server-services-designer-9/Httpin_doc.png)
...
![Swagger documentation link](/resources/Storage/server-services-designer-9/swagger1.png)

A sample swagger doc looks like:

![The generated swagger document](/resources/Storage/server-services-designer-9/swaggerdoc.jpeg)
