...
<https://documentation.neutrinos.com/articles/#!server-services-designer-9/middleware-end-node>

The **Middleware End node **indicates the end of the middleware flow. It is used to send responses back to requests received from a** Middleware start node** or pass the control to the next middleware.
...
- **Response Types**: A drop-down list displaying the types of data the HTTP response returns. If you want to map this field to a bh.input, or bh.local variable, click the **Map button**, select the property, and input the variable name. See [properties](/articles/server-services-designer-9/properties-in-server-services) to learn more.
