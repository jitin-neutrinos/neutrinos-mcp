...
<https://documentation.neutrinos.com/articles/#!components-guide-for-release-6/select>
...
| ![Information](/resources/Storage/components-guide-for-release-6/info.png) | If Group Options is enabled, the following attributes should be set as follows:optionValue: subOption.valueoptionView: {{subOption.viewValue}} |
| --- | --- |
...
1. Input the component field(s) with the attribute value(s):
  - **placeholder** = State
  - **class** = select
  - **optiondata**={{state}} Copy CodeJavaScriptstates: string[]= [
      'karnataka','kerala','andhrapradesh','tamilnadu','delhi','pune'
      ];
2. Save it and run.
3. When the page is loaded the value **placeholder = select** will display the value Example and **class = select** is the class name that can be used to point to a class in a style sheet and **optiondata={{state}}** class will display the list of the options that can be selected.
