...
<https://documentation.neutrinos.com/articles/#!pulse-publication/rule-group>
...
1. Click Rules on the navigation bar. The Rules page contains two tabs: Rule - Displays all the rules created on the platform, and Rule Group - Displays all the rule groups created on the platform.
2. In the **Rules List** page, click the **Add** dropdown button > Choose **Rule Group** as shown in the image below:
    ![add-rule-group](/resources/Storage/pulse-publication/images/add-rule-group.png)
3. The **Rules Details** page allows you to define a **Rule Group**. By default, the Rule Group is untitled. To rename it, click the **Edit** button next to the default name. This page displays key details about the Rule Group, including its version, status, last updated date, created date, and deployment status. It also provides sections for adding a description, defining constants, and managing input and output keys through input-output editors. The image below illustrates the layout of the Rules details page:
    ![](/resources/Storage/pulse-publication/images/rule-designer-rule-group.png)
4. Click the **Rule Designer** tab to open the designer and add decision logic. You can create a new rule logic or import an existing rule or rule group into the designer. To create a rule group, drag and drop a new rule type to define the new rule logic, or drag and drop an existing rule or rule group. The image below illustrates the Rule Designer page for Rule Group:
    ![designer-page-rule-group](/resources/Storage/pulse-publication/images/rule-designer-page-rule-group.png)
5. After placing individual rules on the canvas, connect them in the desired execution order using arrows. The GIF below illustrates how to add and link rules within a Rule Group.
    ![rule-group-page](/resources/Storage/pulse-publication/images/rule-designer-rule-group-page.gif)
...
7. Finally, click the **Save** button to save and publish the Rule Group.
...
![rule-group-add-rulegroup](/resources/Storage/pulse-publication/images/rule-group-add-rulegroup.gif)

| ![Note](/resources/Storage/pulse-publication/project-trailproject/note.png) | When you save and publish the Rule Group, you can either create a new version or overwrite the existing one. |
| --- | --- |
...
![rule-group-with-workflow](/resources/Storage/pulse-publication/images/integrate-rul-group-with-workflow.gif)

[Next Topic](/articles/pulse-publication/workflow)

[Previous Topic](/articles/pulse-publication/configurations)
