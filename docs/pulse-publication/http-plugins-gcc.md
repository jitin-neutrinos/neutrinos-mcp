...
<https://documentation.neutrinos.com/articles/#!pulse-publication/http-plugins-gcc>
...
interface HttpOptions {
  ...
  parseResponseBody?: boolean;     // Whether to parse response body
  url?: string;                    // Target URL (auto-set for non-file requests)
  [key: string]: any;             // Additional request options
}
```
...
```javascript
// Simple POST request
const response = await ap.http('https://api.example.com', {
  path: '/users',
  method: 'POST',
