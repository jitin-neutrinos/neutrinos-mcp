...
<https://documentation.neutrinos.com/articles/#!pulse-publication/trigger-service-gcc>
...
const result = await ap.trigger(action, params, triggerResult, emitLoaderEvent);
...
const apiAction = {
  ...
  config: {
    ...
    method: 'POST',
    body: [{ key: 'name', value: 'John Doe' }]
  }
};

const result = await ap.trigger(apiAction, { eventDetail: {} });
```
...
const response = await ap.trigger(apiConfig, { eventDetail: {} });
...
```javascript
// Reels service trigger
const reelsConfig = {
  type: 'reels',
  config: {
    url: 'https://reels.example.com/data',
    method: 'POST',
    body: [
...
const reelsResult = await ap.trigger(reelsConfig, { eventDetail: {} });
```
...
const cmsResult = await ap.trigger(cmsConfig, { eventDetail: {} });
```
...
const dmsConfig = {
  ...
  config: {
    ...
    validator_token: 'token123',
    body: [
      { key: 'file', value: fileData }
    ]
  }
};

const dmsResult = await ap.trigger(dmsConfig, { eventDetail: {} });
```
...
const caseConfig = {
  ...
  config: {
    ...
    body: [
      { key: 'processId', value: 'proc_123' }
    ]
  }
};

const caseResult = await ap.trigger(caseConfig, { eventDetail: {} });
```
...
await ap.trigger(customCodeConfig, { eventDetail: {} });
```
...
const dialogResult = await ap.trigger(dialogConfig, { eventDetail: {} });
```
...
await ap.trigger(navigateConfig, { eventDetail: {} });
```
