...
<https://documentation.neutrinos.com/articles/#!pulse-publication/reels-datasource-gcc>
...
// Basic reels datasource call
const config = {
  url: 'https://reels.example.com/api/data',
  method: 'POST',
  body: [
...
ap.reelsDs.reels(config, {}, (result) => {
  if (result.error) {
...
```javascript
// Reels call with pagination
const paginationData = {
  pageNumber: 1,
  pageSize: 20,
  pageNumberKey: 'page',
  pageSizeKey: 'size',
  paramType: 'queryParams' // or 'body'
};

const config = {
  url: 'https://reels.example.com/api/users',
  method: 'GET',
  queryParams: [
...
ap.reelsDs.reels(config, paginationData, (result) => {
  if (result.error) {
    console.error('Error:', result.error);
  } else {
    console.log('Users:', result.data);
    console.log('Total pages:', Math.ceil(result.data.total / paginationData.pageSize));
  }
});
```
