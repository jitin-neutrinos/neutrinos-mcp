...
<https://documentation.neutrinos.com/articles/#!pulse-publication/dialog-plugins-gcc>
...
```javascript
// Open a dialog with global page content
await ap.dialog.open({
  page: {
    pageId: 'help-content',
    type: 'global'
...
```javascript
// Open dialog with custom configuration
await ap.dialog.open({
  page: {
    pageId: 'custom-dialog',
    type: 'dialog'
  },
  height: '40%',
  width: '50%',
  header: 'Custom Dialog',
  url: '/custom/pages/fetch'
});
```
