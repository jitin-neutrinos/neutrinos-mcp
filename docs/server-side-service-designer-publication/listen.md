...
<https://documentation.neutrinos.com/articles/#!server-side-service-designer-publication/listen>

The **Listen node** allows you to listen to the event emitted by the [Emit node](/articles/server-side-service-designer-publication/emit). It is a **s****tart **node, but does not initialize the .bh object. Instead, it accepts the .bh object from the** Emit **node.
