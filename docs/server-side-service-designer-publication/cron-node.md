...
<https://documentation.neutrinos.com/articles/#!server-side-service-designer-publication/cron-node>

The **Cron** node is used to schedule a time-based job. That is, it can schedule a job to run periodically at fixed times, dates, or intervals.
...
3. **Expression**: Enter the Cron expression. A Cron expression is a string that describes individual details of the schedule. The string contains the following fields, separated by white space:
    ![cron fields](/resources/Storage/server-side-service-designer-publication/cron_fields.png)
  ...
  - If you enter ***/5 * * * * ***, the job runs every 5 secords.
