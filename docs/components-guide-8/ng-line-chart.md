...
<https://documentation.neutrinos.com/articles/#!components-guide-8/ng-line-chart>
...
- Drag and drop the **NG Line Ch****art** from the Ng Charts category.
...
Let's take the example of representing the views of a series each month of the year.
...
In the **Page Flow Designer **of the page, add a Script node to the On init flow and enter the following code: Copy CodeJavaScriptpage.lineChartData = [
...
page.lineChartLabels = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'June', 'July','Aug','Sep','Oct','nov','dec',];In the Page Variable node of the On Init flow, add the following variables:**Page Variable**
**Default Value**
lineChartData
Select **page**. and enter **lineChartData**
lineChartLabels
Select **page**. and enter **lineChartLabels** Create a **mycharclick **flow. This flow displays an alert pop-up on click of the chart. Drag and drop a **Start **node and name it **mycharclick**. Drag and drop a **Script node and add **alert("chart clicked");Connect the nodes.Navigate to the UI designer of the page. Drag and drop an **Ng Line Chart** component and set the following properties:[datasets]: page.lineChartData [labels]: page.lineChartLabels (chartClick): mychartClickfxFlex: 50Save and run the page.
