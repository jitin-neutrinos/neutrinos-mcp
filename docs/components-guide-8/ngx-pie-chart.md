...
<https://documentation.neutrinos.com/articles/#!components-guide-8/ngx-pie-chart>

## Pie Chart

### Overview

**Pie Chart** is a circular chart that is divided into different slices each of which represents the corresponding data values.
...
- Pharmaceutical Industries (indicating the different sections of drugs)
...
- Drag and drop the **Ngx Pie** **Chart** from the Ngx Charts category.
...
Consider an IT industry that makes a survey on how many users use their products from different parts of the world. The sample data can be recorded as follows:
...
2. Copy CodeJavaScriptpage.pieChartData: Array<any> = [{ "name": "Germany", "value": 40632,},{ "name": "United States", "value": 50000, },{ "name": "France", "value": 36745,},{"name": "Spain", "value": 33000, },{"name": "Italy", "value": 35800, }];
   page.pieTooltipText({ data }) {const label = data.name;const val = data.value;}
3. Create a **mycharclick **flow. This flow displays an alert pop-up on click of the chart.
4. Drag and drop a **Start **node and name it **mycharclick**.
5. Drag and drop a **Script node and add **alert("chart clicked");
6. Connect the nodes.
7. Navigate to the UI designer of the page.
8. Drag and drop an **Ngx Pie Chart** component and set the following properties:
  1. [result]: page.pieChartData
  2. [tooltipText]: page.pieTooltipText
  3. (chartClick): mychartClick
  4. showLabels: True
  5. fxFlex: 50
