...
<https://documentation.neutrinos.com/articles/#!alpha-platform/editors-inbox>
...
![inbox-complete](/resources/Storage/alpha-platform/images/inbox-complete.png)
...
| ![Note](/resources/Storage/alpha-platform/project-trailproject/note.png) | The **Inbox** is scoped per project. Each project includes a **My** **Task Inbox**, **Team Inbox**, and **Groups Inbox **shared among all users. Additionally, an **Enquiry Inbox** can be enabled if needed. |
| --- | --- |
...
1. **My Task**: This type of Inbox is designed to display tasks specifically assigned to the logged-in Workbench user. It enables them to focus on their individual responsibilities, streamline task prioritization based on urgency or deadlines, and quickly access task details and related Case Information. The image below illustrates My Tasks inbox in Workbench:
    ![](/resources/Storage/alpha-platform/images/mytasks-inbox.png)
2. **Team Tasks**: This Inbox displays tasks assigned to the reportees of the logged-in Workbench user. The image below illustrates Team Tasks inbox in Workbench:
    ![](/resources/Storage/alpha-platform/images/teamtasks-inbox.png)
3. **Group Tasks**: Displays tasks assigned to a specific group of users but not yet allocated to any individual team member. The image below illustrates Group Tasks inbox in Workbench:
    ![](/resources/Storage/alpha-platform/images/grouptasks-inbox.png)
4. **Admin Inbox**: The Admin Inbox is similar to a standard user inbox but includes additional features that enable administrators to effectively manage and coordinate cases and tasks assigned to team members, as well as monitor their progress. The Admin can access three types of inboxes as discussed below:
  1. Tasks Inbox: This inbox type displays all the cases linked with the project. The image below illustrates Tasks inbox showcasing all the tasks for a particular project:
      ![tasks-inbox-fullscreen.png](/resources/Storage/alpha-platform/images/admin-tasks-inbox-fullscreen.png)
  2. Signal Inbox: If a deployed BPM process contains a single task and the Save Signal API is triggered before it, this list will display signals that are in a waiting state. The Alpha Admin can manually trigger this signal to approve the task, allowing the process to resume to the next step. The image below illustrates the Signal inbox in Admin UI Workbench.
      ![](/resources/Storage/alpha-platform/images/adminui-signal-inbox.png)
  3. Re-attempt Inbox: The Re-attempt Inbox in the Admin UI of Neutrinos Alpha Workbench is designed to manage failed or incomplete tasks. Based on business requirements, specific tasks can be configured to appear as 'Re-attempt.' This scenario enables an admin to restart the process from the exact node where it previously failed. The image below illustrates a Re-attempt inbox in Admin UI Workbench.
      ![](/resources/Storage/alpha-platform/images/adminui-re-attempt-inbox.png)
...
| ![Warning](/resources/Storage/alpha-platform/warning.png) | The 'My Tasks' inbox must always be enabled for the logged-in user to view their assigned tasks. |
| --- | --- |

![workflow-studio-config-disable-inbox](/resources/Storage/alpha-platform/images/workflow-studio-config-disable-inbox.png)

| ![Note](/resources/Storage/alpha-platform/project-trailproject/note.png) | If an inbox is disabled, any other inbox-related settings will not take effect, as the inbox is not accessible to the user. |
| --- | --- |

The image below shows that Team Tasks inbox is unavailable to Workbench users when the toggle is turned off:

![](/resources/Storage/alpha-platform/images/workflow-studio-config-disable-inbox-workbench.png)

| ![Note](/resources/Storage/alpha-platform/project-trailproject/note.png) | For Inbox, you can add custom code trigger. For more information see, [Custom Code](/articles/alpha-platform/triggers/a/h2_1762374550). |
| --- | --- |
...
| ![Note](/resources/Storage/alpha-platform/project-trailproject/note.png) | When the filter is disabled, users will not be able to view the filter options in the Inbox. |
| --- | --- |

The below image illustrates the toggle to enable filtering functionality in the My Tasks Inbox.

![](/resources/Storage/alpha-platform/images/workflow-studio-config-enable-filter.png)
...
The GIF below, illustrates adding the filter criteria to Team Tasks inbox:
 ...
 ![](/resources/Storage/alpha-platform/images/workflow-studio-config-add-filter-gif.gif)
...
![](/resources/Storage/alpha-platform/images/workflow-studio-config-enable-filter-workbench.png)
...
![](/resources/Storage/alpha-platform/images/workflow-studio-config-save-filter.png)
...
![](/resources/Storage/alpha-platform/images/workflow-studio-config-delete-filter.gif)
...
![](/resources/Storage/alpha-platform/images/columns-gif.gif)
...
![](/resources/Storage/alpha-platform/images/workflow-studio-config-add-column-gif.gif)
...
![](/resources/Storage/alpha-platform/images/workflow-studio-config-add-column-workbench.png)
...
![](/resources/Storage/alpha-platform/images/workflow-studio-config-column-selection-toggle.png)
...
| ![Note](/resources/Storage/alpha-platform/project-trailproject/note.png) | The user can sort column content only if the **Table Sort** toggle is enabled. If this toggle is disabled, sorting will not be allowed, even if the column configuration permits it. |
| --- | --- |

The image below shows the Table Sort toggle enabled for Team Tasks inbox:

![](/resources/Storage/alpha-platform/images/workflow-studio-config-table-sort.png)
...
![](/resources/Storage/alpha-platform/images/workflow-studio-config-set-number-of-row-gif.gif)
...
![](/resources/Storage/alpha-platform/images/workflow-studio-config-set-number-of-row-workbench.png)

| ![Note](/resources/Storage/alpha-platform/project-trailproject/note.png) | Note that the default number of rows also depends on the default inbox list configured in Workbench. For example, if the default list is 'My Tasks' inbox with 5 rows set, the same number of rows will be shown by default in all other inbox types. |
| --- | --- |
...
![](/resources/Storage/alpha-platform/images/workflow-studio-config-dashboard-behavior.png)
...
![](/resources/Storage/alpha-platform/images/workflow-studio-config-dashboard-add-card-gif.gif)
...
![](/resources/Storage/alpha-platform/images/workflow-studio-config-dashboard-add-card-workbench.png)
...
![workflow-studio-config-case-summary-download-gif](/resources/Storage/alpha-platform/images/workflow-studio-config-case-summary-download-gif.gif)

The GIF below illustrates how user can download case summary from Workbench:

![workflow-studio-case-summary-download-gif](/resources/Storage/alpha-platform/images/workflow-studio-case-summary-download-gif.gif)
...
| ![Note](/resources/Storage/alpha-platform/project-trailproject/note.png) | The Force Re-assignment toggle, when enabled, allows tasks to be assigned to users even if their bandwidth is set to 0. This is particularly useful for high-priority tasks that require immediate completion. |
| --- | --- |

The image below shows configurations enabled for re-assignment from Team Tasks inbox:

![workflow-studio-config-reassignment](/resources/Storage/alpha-platform/images/workflow-studio-config-reassignment.png)
...
![workflow-studio-enquiry-inbox-global-page-gif](/resources/Storage/alpha-platform/images/workflow-studio-enquiry-inbox-global-page-gif.gif)
