...
<https://documentation.neutrinos.com/articles/#!alpha-platform/toasts>
...
```javascript
interface ToastOptions {  status: string;      // Toast status (success, error, warning, info)  title: string;       // Toast title  message: string;     // Toast message content  duration?: number;   // Display duration in milliseconds}
```
...
```javascript
// Toast configuration with translation keys{  message: {    summary: '{{lang.toast.success.title}}',    detail: '{{lang.toast.success.message}}'  }}
```
...
```javascript
// Show a simple toastap.toast.showToast({  status: 'success',  title: 'Success!',  message: 'Operation completed successfully',  duration: 5000});
```
