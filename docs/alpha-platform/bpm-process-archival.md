...
<https://documentation.neutrinos.com/articles/#!alpha-platform/bpm-process-archival>
...
For end-to-end guidance on planning archival and restore operations, refer to the [Use Case](/articles/alpha-platform/bpm-use_case-run-book) topic.
...
| ![Note](/resources/Storage/alpha-platform/project-trailproject/note.png) | Do not commit secrets. Prefer local `.env` files and environment injection in CI/CD. |
| --- | --- |

## Table of Contents

- [Authentication](/articles/alpha-platform/bpm-process-archival/a/h2__55841322)
- [Operational Checklist](/articles/alpha-platform/bpm-process-archival/a/h2__424495486)
- [Endpoints](/articles/alpha-platform/bpm-process-archival/a/h2__691415180)
  - [Count Processes](/articles/alpha-platform/bpm-process-archival/a/h3_1035812684)
  - [Get Audit Logs](/articles/alpha-platform/bpm-process-archival/a/h2__168061396)
  - [Find Processes](/articles/alpha-platform/bpm-process-archival/a/h2__1164346748)
  - [Find Child Processes](/articles/alpha-platform/bpm-process-archival/a/h2_1025542618)
  - [Delete Processes](/articles/alpha-platform/bpm-process-archival/a/h2__2100547968)
  - [Generate SQL Backup](/articles/alpha-platform/bpm-process-archival/a/h2_1907204723)
  - [Backup Processes](/articles/alpha-platform/bpm-process-archival/a/h2_2135229242)
  - [Restore Processes](/articles/alpha-platform/bpm-process-archival/a/h2_2135229242)
...
```code
{    "processName": "order-fulfillment",    "processStatus": "ACTIVE",    "deploymentId": "dep-54321",    "beforeNoOfDays": 30,    "taskName": "Process Payment",    "taskStatus": "PENDING",    "taskAssignee": "john.doe@example.com",    "processVariables": [{ "name": "customerId", "value": "CUST-12345" }],    "piids": [1001, 1002, 1003]}
```
...
```code
curl -X POST \  "http://{domain_name}/archivalservice/bpm/processes/count" \  -H "Authorization: Bearer <token>" \  -H "Content-Type: application/json" \  -d '{    "processName": "order-fulfillment",    "beforeNoOfDays": 30  }'
```
...
```code
curl -X GET \  "http://{domain_name}/archivalservice/bpm/processes/audits/a4c1e8f2-56c0-4a2d-9a05-12e1c90a8d77" \  -H "Authorization: Bearer <token>"
```

Response (200):

```code
[    {        "auditId": "a4c1e8f2-56c0-4a2d-9a05-12e1c90a8d77",        "auditType": "backup",        "auditStatus": "started",        "remainingProcessCount": 200,        "batchName": "process_count",        "batchStatus": "inprogress",        "createdAt": "2025-01-01T12:00:00.000Z"    }]
```
...
```code
curl -i -X GET \  "http://{domain_name}/archivalservice/bpm/processes/audits?auditType=backup&processName=customerOnboarding&pageNumber=1&pageSize=20" \  -H "Authorization: Bearer <token>"
```
...
```code
curl -X GET \  "http://{domain_name}/archivalservice/bpm/processes/audits/summary?auditType=backup&from=2025-01-01T00:00:00Z&to=2025-02-01T00:00:00Z" \  -H "Authorization: Bearer <token>"
```
...
```code
{    "processVariables": [        { "name": "status", "value": "OPEN" },        { "name": "", "value": "APPROVED" },        { "name": "priority", "value": "" }    ]}
```
...
```code
{    "processName": "customerOnboarding",    "processStatus": "COMPLETED",    "deploymentId": "dep-xyz-789",    "beforeNoOfDays": 30,    "taskName": "approveApplication",    "taskStatus": "PENDING",    "taskAssignee": "john.doe@example.com",    "processVariables": [{ "name": "customerId", "value": "CUST-12345" }],    "piids": [2001, 2002],    "pageSize": 50,    "pageNumber": 1}
```
...
```code
curl -X POST \  "http://{domain_name}/archivalservice/bpm/processes" \  -H "Authorization: Bearer <token>" \  -H "Content-Type: application/json" \  -d '{    "processName": "customerOnboarding",    "processStatus": "COMPLETED",    "pageSize": 50,    "pageNumber": 1  }'
```
...
```code
{    "processName": "order-fulfillment",    "pageNumber": 2147483648}
```

**Example Response:**

```code
{    "status": 500,    "message": "value \"2147483648\" is out of range for type integer"}
```
...
```code
curl -X POST \  "http://{domain_name}/archivalservice/bpm/processes/children/find" \  -H "Authorization: Bearer <token>" \  -H "Content-Type: application/json" \  -d '{ "parentProcessInstanceIds": [1001, 1002] }'
```
...
```code
{    "statusCode": 400,    "message": ["each value in parentProcessInstanceIds must be an integer number"]}
```
...
```code
curl -X POST \  "http://{domain_name}/archivalservice/bpm/processes/delete-bulk" \  -H "Authorization: Bearer <token>" \  -H "Content-Type: application/json" \  -d '{ "processInstanceIds": [12345, 67890] }'
```
...
```code
{    "statusCode": 400,    "message": ["each value in processInstanceIds must be an integer number"]}
```
...
```code
{    "statusCode": 400,    "message": ["processInstanceIds should not be empty"]}
```
...
```code
curl -X POST \  "http://{domain_name}/archivalservice/bpm/processes/generate-sql" \  -H "Authorization: Bearer <token>" \  -H "Content-Type: application/json" \  -d '{ "processInstanceIds": [1001, 1002, 1003] }'
```
...
```code
{    "statusCode": 400,    "message": ["each value in processInstanceIds must be an integer number"],    "error": "Bad Request"}
```
...
```code
{    "statusCode": 400,    "message": ["processInstanceIds should not be empty"],    "error": "Bad Request"}
```
...
```code
{    "processName": "customerOnboarding",    "processStatus": "COMPLETED",    "deploymentId": "dep-xyz-789",    "beforeNoOfDays": 30,    "taskName": "approveApplication",    "taskStatus": "PENDING",    "taskAssignee": "john.doe@example.com",    "processVariables": [{ "name": "customerId", "value": "CUST-12345" }],    "piids": [2001, 2002, 2003]}
```
...
```code
curl -X POST \  "http://{domain_name}/archivalservice/bpm/processes/backup" \  -H "Authorization: Bearer <token>" \  -H "Content-Type: application/json" \  -d '{    "processName": "customerOnboarding",    "processStatus": "COMPLETED",    "beforeNoOfDays": 30  }'
```

### Curl (Restore)

```code
curl -X POST \  "http://{domain_name}/archivalservice/bpm/processes/restore" \  -H "Authorization: Bearer <token>" \  -H "Content-Type: application/json" \  -d '{    "processName": "customerOnboarding",    "processStatus": "COMPLETED",    "beforeNoOfDays": 30  }'
```

### Response

```code
{    "message": "Backup/Restore initiated successfully",    "auditId": "3f49e17a-5121-4f2e-86e2-8d2e3f45c91f"}
```
