...
<https://documentation.neutrinos.com/articles/#!alpha-platform/json-evaluator-plugin>
...
```javascript
const data = {  users: [    { name: 'John', age: 30, city: 'New York' },    { name: 'Jane', age: 25, city: 'Los Angeles' }  ]};// Extract specific valuesconst userName = await ap.jval.get(data, 'users[0].name');        // 'John'const userAge = await ap.jval.get(data, 'users[1].age');         // 25const allNames = await ap.jval.get(data, 'users.name');          // ['John', 'Jane']
```
