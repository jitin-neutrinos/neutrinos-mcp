...
| **AD-13** | Distribution via `install.sh`/`install.ps1` + a daily CI rebuild published as a GitHub release; the running server refreshes its index from that release in a background thread, once per process, never on the request path | Ship the repo only; rebuild locally per install | The index build takes ~25 min and needs a live crawl — most installs should not pay that cost. **CONFIRMED 2026-08-26, found and fixed during review**: the first implementation ran the update check synchronously inside the first tool call, with only a per-socket `timeout=` bound; on a network that stalls rather than refuses (a proxy or firewall, not a clean "no route"), that blocked the call past the MCP client's own timeout and tore down the whole stdio connection — this is what broke the session in practice, not a retrieval bug. Fixed by moving the check to a fire-and-forget daemon thread started once from `main()`, and by validating a downloaded file's `build_manifest` (AD-12) and backing up the existing DB to `.prev` before replacing it, since GitHub release metadata carries no checksum or signature to check instead. A second, unrelated Windows finding surfaced building `install.ps1`: the same execution policy that blocks `pip.exe` on a locked-down machine also blocks pip's generated `.exe` console-script launchers (`neutrinos-mcp.exe` etc.) — confirmed by actually running one and getting the identical `Access is denied`. Both scripts (and the README's manual fallback) therefore register the server via `python.exe -m neutrinos_mcp.server`, never the generated executable. A third finding, from an actual failed install: a stale `~/.neutrinos-mcp` left by an earlier interrupted run made `git clone` refuse to run at all ("already exists and is not an empty directory") — the installer had no way to tell "complete prior install" from "partial wreckage" apart from the directory merely existing. Fixed in both scripts with the transactional pattern researched from rustup/deno-style installers (`trap ... EXIT` in bash; `try`/`finally` + explicit `$LASTEXITCODE` checks in PowerShell, since `$ErrorActionPreference = "Stop"` does not cover native-command failures — the exact gap behind a filed, confirmed issue against Claude Code's own `install.ps1`, github.com/anthropics/claude-code/issues/26880): a `.install_complete` marker, written only as the last step of a fully successful run, is what "directory exists" is actually checked against. Present → update in place (`git pull`). Absent → treated as unsafe wreckage, removed before cloning fresh. A failure partway through a fresh (or wreckage-replacing) install removes everything it created, leaving nothing behind to block the next attempt; a failure partway through updating an already-complete install leaves that install untouched — a bad DB fetch or a missing `claude` CLI must never cost you a working local install. All three paths (fresh-install failure, stale-wreckage detection, complete-install survival) were verified directly against a local throwaway git remote for both scripts before this was pushed. Both scripts also register with **Claude Desktop** (`claude_desktop_config.json`, merged via a small Python script rather than overwritten, since that file commonly already holds other MCP servers) — researched rather than assumed: **Cowork**, the agentic-work tab in the Claude Desktop app, is not a separate app and has no config of its own; Desktop's SDK layer bridges servers registered in its config into Cowork's sandboxed VM automatically, while a server added directly inside a Cowork session cannot connect at all (the VM is isolated from the host) — confirmed against Anthropic's own support docs and a third-party writeup of the (at the time undocumented) bridging mechanism. So Desktop's config is the one registration target that covers both surfaces |
